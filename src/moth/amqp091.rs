...
impl Moth for Amqp091 {
    ...
    async fn publish(&mut self, exchange: &str, topic: &str, msg: &Message) -> Result<()> {
        ...
        if let Some((raw_body, headers_map, content_type)) = crate::postformat::export_any(msg, version, &options) {
            let mut headers = FieldTable::default();
            for (k, v) in headers_map {
                // Topic is derived internally and placed into headers map by export_any
                if k != "topic" {
                    headers.insert(k.into(), lapin::types::AMQPValue::LongString(v.into()));
                }
            }
            
            // Using the explicit topic if provided, else attempt to get from derived headers (which would have been generated by postformat)
            // Python's behavior replaces '*' and '#' in the routing_key (topic).
            let safe_topic = topic.replace("#", "%23").replace("*", "%22");
